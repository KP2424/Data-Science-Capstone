...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        # For all sites, create a pie chart showing success counts
        ...
        fig = px.pie(
            ...
            color='class',
            color_discrete_map={1: '#7CFC00', 0: '#FF5733'}
        )
        
        # Update labels for better readability
        fig.update_traces(labels=['Success' if val == 1 else 'Failure' for val in success_counts['class']])
        return fig
    else:
...
def update_scatter(selected_site, payload_range):
    ...
    # Create scatter plot based on site selection
    if selected_site == 'ALL':
        ...
        fig = px.scatter(
            ...
            hover_data=['Launch Site', 'Booster Version', 'Payload Mass (kg)', 'class']
        )
    else:
        ...
        fig = px.scatter(
            ...
            labels={'Payload Mass (kg)': 'Payload Mass (kg)', 'class': 'Launch Outcome'},
            hover_data=['Booster Version', 'Payload Mass (kg)', 'class']
        )
